#!/usr/bin/env python3
"""Generate PowerPoint for Q3 exam strategy."""
...
OUT = os.path.join(BASE, "q3_exam_strategy.pptx")
...
add_text_box(slide, Inches(1), Inches(2.0), Inches(11.3), Inches(1),
             "Data Lake Integration Strategy",
             font_size=44, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
add_text_box(slide, Inches(1.5), Inches(3.3), Inches(10.3), Inches(0.7),
             "S3 Data Lake  |  AWS Glue  |  Athena  |  Redshift  |  Hive  |  QuickSight",
...
add_text_box(slide, Inches(2), Inches(4.8), Inches(9.3), Inches(0.5),
             "Exam Strategy  —  Question 3  —  LEAST operational overhead",
...
scenario = (
    "A financial services company maintains a vast and secure data lake built on Amazon S3. "
    "They analyze this data using a diverse set of tools, namely Apache Hive, Amazon Athena, "
    "Amazon Redshift, and Amazon QuickSight. The company has tasked you with identifying a "
    "solution to integrate the data lake and the above tools used by the engineering teams."
)
...
options_detail = [
    {
        "file": "option_a_glue_catalog.png",
        "letter": "A",
        "title": "AWS Glue Data Catalog",
        "verdict": "CORRECT",
        "is_correct": True,
        "bullets": [
            ("Glue crawlers auto-discover schema from S3 and populate a centralized metadata catalog", False),
            ("Single catalog shared by Athena, Redshift Spectrum, Hive, and QuickSight — zero duplication", True),
            ("Fully serverless — no infrastructure to manage, patch, or scale", True),
            ("Schema changes propagate automatically to all tools via the shared catalog", False),
            ("Drop-in replacement for Apache Hive metastore — native integration with all listed tools", False),
        ],
    },
    {
        "file": "option_b_direct_connect.png",
        "letter": "B",
        "title": "Direct S3 Connection per Tool",
        "verdict": "INCORRECT",
        "is_correct": False,
        "bullets": [
            ("Each tool must define its own schema, table definitions, and IAM access independently", False),
            ("Significant operational overhead — schema must be maintained separately in every tool", True),
            ("No shared metadata catalog — risk of schema drift and inconsistencies across tools", True),
            ("Any schema change requires updating configuration in all four tools manually", False),
        ],
    },
    {
        "file": "option_c_ec2_server.png",
        "letter": "C",
        "title": "EC2 Central Analysis Server",
        "verdict": "INCORRECT",
        "is_correct": False,
        "bullets": [
            ("Requires managing EC2 instances: OS patching, scaling, storage, backups", False),
            ("Adds unnecessary infrastructure overhead — the opposite of LEAST operational overhead", True),
            ("Data must be copied from S3 to EC2 storage — transfer cost and staleness risk", True),
            ("AWS Glue provides the same centralization benefit as a fully managed serverless service", False),
        ],
    },
    {
        "file": "option_d_copy_data.png",
        "letter": "D",
        "title": "Upload Copies to Each Tool",
        "verdict": "INCORRECT",
        "is_correct": False,
        "bullets": [
            ("Creates 4 separate copies of the data — massive storage cost increase", False),
            ("Data duplication leads to inconsistencies when source data changes", True),
            ("Must build and maintain 4 separate sync pipelines — highest operational overhead", True),
            ("Completely unnecessary when Glue Catalog lets all tools query the same S3 data", False),
        ],
    },
]
...
answers = [
    ("A", "AWS Glue Data Catalog",   "CORRECT",   CORRECT_GRN,
     "Serverless, centralized catalog — all tools share one metadata source with zero infra overhead."),
    ("B", "Direct S3 per Tool",      "INCORRECT", INCORRECT_R,
     "Each tool configured separately — duplicate schema effort, drift risk, high maintenance."),
    ("C", "EC2 Analysis Server",     "INCORRECT", INCORRECT_R,
     "Self-managed EC2 — OS patching, scaling, data copy overhead. Opposite of least overhead."),
    ("D", "Copy Data to Each Tool",  "INCORRECT", INCORRECT_R,
     "4x data duplication — storage cost, inconsistency, 4 sync pipelines to maintain."),
]
