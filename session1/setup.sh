...
port_in_use() {
    if command -v ss &>/dev/null; then
        ...
        # Fallback: parse /proc/net/tcp (port in hex)
        local hex_port
...
start_app() {
    ...
    # Wait for the port to become active (up to 15 seconds)
    local waited=0
    ...
    # Process alive but port not confirmed — still OK
    if kill -0 "$app_pid" 2>/dev/null; then
