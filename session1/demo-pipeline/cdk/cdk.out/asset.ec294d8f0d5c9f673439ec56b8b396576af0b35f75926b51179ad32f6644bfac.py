...
if record_count > 0:
    ...
    # Using boto3 instead of Spark SQL to avoid catalog connectivity issues
    import boto3
    glue_client = boto3.client("glue")
    try:
        glue_client.start_crawler(Name="dea-datalake-crawler")
        print("Triggered crawler to update partitions.")
    except Exception as e:
        print(f"Crawler trigger skipped (may already be running): {e}")
        print("Partitions will be available after next crawler run or manual MSCK REPAIR TABLE in Athena.")
else:
